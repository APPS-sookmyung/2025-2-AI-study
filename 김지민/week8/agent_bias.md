...
## Bias
LLM-as-a-Judge를 실제로 돌려보면, 사람과 많이 맞더라도 여러 **편향(bias)**이 드러남.
**모델 자체가 가진 편향 + judge 세팅 특유의 편향**이 합쳐서 나타냠
1. Task-agnostic biases (태스크와 무관한 일반 편향)
- LLM 전반에 존재하는 편향 (성별, 인종, 문화 등)
- Judge가 자기가 만든 답을 더 높게 평가하는 경향

2. Judgment-specific biases (판사 역할에서 더 심하게 나타나는 편향)
- 위치 편향
- 보기 좋은 포맷, 이모지, 친근한 톤, 감정적인 어조 등 judge가 특정 스타일을 더 선호
- 구체적인 숫자, 인용, 기관명, 논문, 용어 등이 들어간 답을 더 좋아하는 경향
- 더 긴 응답을 더 좋은 응답으로 평가하는 경향

### 실제 효과 좋은 기법들
1. 여러 번 돌리고 majority voting 하는 방식
같은 프롬프트로 5번 평가시키고, 그 중 가장 많이 나온 선택을 최종 결과로 쓰는 전략

2. 여러 LLM을 함께 쓰고 투표

3. Pairwise 비교 + 위치 바꿔서 평가(swap)하는 전략

### 실제 효과 안 좋은 기법들
1. 점수 + 설명(reason)을 같이 출력 -> 점수 + 설명을 한 번에 뽑는 프롬프트”는 성능 개선 측면에서는 비추!!!!!

2. Self-validation (자기 평가 다시 묻기)
Judge가 먼저 점수를 낸 다음 → “이 평가에 확신 있냐?” 같은 self-check를 시키는 전략
- GPT-3.5 기준으로 보면 효과 거의 없음.
- 모델이 **과신(overconfidence)**하는 경향 때문에 자기 평정을 크게 수정하려 하지 않는다는 분석.

3. Multi-round 평균/최댓값
majority@5처럼 여러 번 평가하긴 하는데, **평균 점수(mean@5)**나 **가장 높은 점수(best-of-5)**를 쓰는 방식.
-> “여러 번 돌리면 무조건 좋다”가 아니고, “다수결(majority)”만 의미 있는 개선, mean/best는 효과 미비 또는 역효과.